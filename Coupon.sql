SELECT DISTINCT
  ...
  CAST(o.`created_at`AS DATE) AS 'Created Time',
...
ON sst.shipment_status_code = ss.udropship_status 
 LEFT JOIN `aggregate`.`product_cat_final` pcf
  ...
  WHERE UPPER(o.`coupon_code`) LIKE '%LOTTEMART%' OR UPPER(o.`coupon_code`) LIKE '%FLEMINGTON%' OR UPPER(o.`coupon_code`) LIKE '%LUGIA%'
